...
interface MessageDetailOrganizerCancelMeetingProps {
  notification: NotificationItem;
  onClose: () => void;
}

export default function MessageDetailOrganizerCancelMeeting({ notification, onClose }: MessageDetailOrganizerCancelMeetingProps) {
  ...
  const messageText = notification.notification_text || 
    (notification.meeting_title
      ? `Вы отменили встречу "${notification.meeting_title}", которая должна была пройти с ${startDate} по ${endDate}. Мы очень сожалеем, что у вас не получилось провести встречу.`
      : 'Вы отменили встречу. Мы очень сожалеем, что у вас не получилось провести встречу.');
