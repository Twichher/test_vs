...
TRUNCATE TABLE faq_table_18 RESTART IDENTITY;

INSERT INTO faq_table_18 (question_text, question_answer) VALUES
  (
      'Что такое VSTRECHI.RU и для чего он нужен?',
      'VSTRECHI.RU — это платформа для организации и посещения офлайн-встреч в Москве. Здесь вы можете находить интересные мероприятия по районам, записыватьс
  я на них, создавать собственные встречи и находить единомышленников.'
  ),
  (
      'Как зарегистрироваться на платформе?',
      'Для регистрации нажмите кнопку «Регистрация» в правом верхнем углу, заполните форму с email, паролем и базовыми данными (имя, фамилия, район, дата рожд
  ения). После подтверждения email вы сможете войти в систему и пользоваться всем функционалом.'
  ),
  (
      'Что такое «валюта встреч» и как её получить?',
      '«Валюта встреч» — это внутренняя единица платформы. Каждый новый пользователь получает стартовый баланс. Также валюта начисляется организаторам за пров
  едённые встречи. Если валюты не хватает, вы можете докупить её в магазине услуг.'
  ),
  (
      'Как записаться на встречу и что нужно для этого?',
      'На странице «Встречи» выберите интересующее мероприятие и нажмите «Записаться». Для записи требуется достаточное количество валюты встреч — за каждое а
  ктивное участие (запись на созданную или идущую встречу) резервируется 1 единица валюты.'
  ),
  (
      'Как стать организатором встреч?',
      'Есть два способа: купить роль организатора в магазине услуг, либо дождаться, пока администрация платформы выдаст вам этот статус. Организатор может соз
  давать встречи, управлять списком участников и получать валюту за проведённые мероприятия.'
  ),
  (
      'Что будет, если я пропущу встречу, на которую записался?',
      'Если вы не явились на встречу, организатор может отметить вас как отсутствующего. В этом случае резервированная валюта списывается, а ваш рейтинг как г
  остя может снизиться. При частых пропусках аккаунт может быть заблокирован.'
  ),
  (
      'Организатор отметил меня как отсутствующего, но я был на встрече. Что делать?',
      'Если вы считаете, что организатор ошибся, нажмите «Да, я был» в уведомлении о конфликте. После этого другие участники встречи проголосуют — если больши
  нство подтвердит ваше присутствие, статус изменится на «присутствовал», и валюта не спишется. Если голосование проиграно, вы можете подать апелляцию с доказ
  ательствами.'
  ),
  (
      'Как работает система рейтингов и оценок пользователей?',
      'После каждой встречи участники и организатор оценивают друг друга по шкале от 1 до 10. Рейтинг формируется как среднее арифметическое всех оценок. Такж
  е учитывается промежуточный рейтинг за текущий период. Высокий рейтинг повышает доверие к пользователю.'
  ),
  (
      'Можно ли купить дополнительные встречи или роль организатора?',
      'Да, в магазине услуг вы можете приобрести пакеты встреч (например, 1, 5 или 10 встреч) или роль организатора. После покупки баланс или статус обновится
  в течение минуты.'
  ),
  (
      'Как изменить свои личные данные (имя, фамилию, район, фото)?',
      'Перейдите в свой профиль по кнопке в левом меню, затем нажмите на шестерёнку «Настройки». Там вы можете изменить имя, фамилию, дату рождения, пол, райо
  н и загрузить фото профиля. Некоторые данные могут требовать повторной верификации.'
  );
...
insert into services_table_19 (service_name, service_description, service_price)
values 
('1 встреча', 'Данная покупка позволит Вам посетить 1 встречу', 100),
('3 встречи', 'Данная покупка позволит Вам посетить 3 встречи', 300),
('5 встреч', 'Данная покупка позволит Вам посетить 5 встреч', 500),
('10 встреч', 'Данная покупка позволит Вам посетить 10 встреч', 1000),
('Роль организатора', 'Данная покупка позволит Вам создавать собственные встречи и зарабатывать на этом', 5000)
...
INSERT INTO categories_to_support_table_23 (text_category) VALUES
  ('Баг / ошибка в работе сервиса'),
  ('Несправедливо списали встречу'),
  ('Обман / мошенничество'),
  ('Неприемлемое поведение пользователя'),
  ('Проблема с оплатой или покупкой услуги'),
  ('Проблема с верификацией аккаунта'),
  ('Жалоба на организатора встречи'),
